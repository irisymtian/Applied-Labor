...
pay_gap_adjusted_did <- feols(
    logy ~ female * post + age + I(age^2) + I(age^3) | worker_id + year,
    data = wage_long,
    cluster = ~worker_id
)
...
firm_premium_worker_year_did <- feols(
    psi_hat ~ female * post | worker_id + year,
    data = did_worker_regression_data,
    cluster = ~worker_id
)
# can we control for sector/industry?
...
did_regression_extra_rows <- data.frame(
    ...
    `log(hourly wage)` = c("by: worker_id", "year"),
    `firm premia` = c("by: worker_id", "x"),
    `firm premia` = c("by: worker_id", "year"),
    check.names = FALSE
)
