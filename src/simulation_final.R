...
# 1. simulate the number of workers move from firm k to firm j to recover the value of firm j (V_j) with Sorkin's method (tilde_exptV.m);
# 2. for each firm, there should be firm wage premia & worker's value of the firm (latent)
# 3. the key is to test for the correlation between simulated real value & recovered value
...
#  different wage premium (normaly lower for women) and common non-wage factors but have different value
...
# Save cache and outputs inside the repository.
...
cache_dir <- file.path(project_dir, "simulated_data")
output_dir <- file.path(project_dir, "results", "simulation")
dir.create(cache_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
simulation_cache <- file.path(cache_dir, "simulated_data_cache.rds")
...
data_summary_by_year_gender_neat <- data_summary_by_year_gender %>%
    transmute(
        ...
        `EE rate` = ee_rate,
        `ENE rate` = ene_rate,
        `Mean log wage` = mean_log_wage,
        `Mean AKM psi` = mean_akm_psi,
        `Mean true psi` = mean_true_psi
    )
