...
pay_gap_raw_did_1 <- feols(
  logy ~ female * post,
  data = wage_long,
  cluster = ~worker_id
)

pay_gap_raw_did_2 <- feols(
    logy ~ female * post | year,
    data = wage_long,
    cluster = ~worker_id
)

pay_gap_raw_did_3 <- feols(
  logy ~ female * post | year + worker_id,
  data = wage_long,
  cluster = ~worker_id
)
...
firm_premium_worker_did_1 <- feols(
    psi_hat ~ female * post,
    data = did_worker_regression_data,
    cluster = ~worker_id
)

firm_premium_worker_did_2 <- feols(
  psi_hat ~ female * post | year,
  data = did_worker_regression_data,
  cluster = ~worker_id
)

firm_premium_worker_did_3 <- feols(
  psi_hat ~ female * post | year + worker_id,
  data = did_worker_regression_data,
  cluster = ~worker_id
)
...
did_regression_models <- list(
    "log(hourly wage)" = pay_gap_raw_did_1,
    "log(hourly wage)" = pay_gap_raw_did_2,
    "log(hourly wage)" = pay_gap_raw_did_3,
    "log(hourly wage)" = pay_gap_adjusted_did,
    "firm premia" = firm_premium_worker_did_1,
    "firm premia" = firm_premium_worker_did_2,
    "firm premia" = firm_premium_worker_did_3,
...
did_regression_extra_rows <- data.frame(
    ...
    `log(hourly wage)` = c("by: worker_id", "x", "x", "", "", ""),
    ...
    `firm premia` = c("by: worker_id", "x", "x", "", "", ""),
...
if (has_modelsummary) {
    modelsummary::modelsummary(
        ...
        vcov = rep(list(~worker_id), length(did_regression_models)),
